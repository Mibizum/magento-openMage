 ...
 * by Mibizum_Sync_Helper_SmartItem::fetchSmartItem(). 404 if it does not exist
...
class Mibizum_Sync_IndexController extends Mage_Core_Controller_Front_Action
{
    ...
    {
        parent::preDispatch();
        if (!Mage::helper('mibizum_sync/smartItem')->isFichaEnabled()) {
            $this->norouteAction();
    ...
    public function viewAction()
    {
        ...
        $smartItem = Mage::helper('mibizum_sync/smartItem')->fetchSmartItem($slug);
        if (!$smartItem) {
            $this->norouteAction();
            return;
        }
