<?php
/**
 * Grid for the Attribute Badges listing.
 *
 * Compatible with PHP 5.4+.
 */
class Mibizum_Sync_Block_Adminhtml_Attributebadge_Grid extends Mage_Adminhtml_Block_Widget_Grid
{
    public function __construct()
    {
        parent::__construct();
        $this->setId('mibizum_sync_attributebadge_grid');
    ...
    {
        $collection = Mage::getModel('mibizum_sync/attributeBadge')->getCollection();
    ...
    protected function _prepareColumns()
    {
        ...
        $this->addColumn('action', array(
            ...
            'actions'   => array(array(
                'caption' => $h->__('Edit'),
                'url'     => array('base' => 'adminhtml/mibizum_sync_attributebadge/edit'),
                'field'   => 'id',
            )),
        ));
    ...
    protected function _prepareMassaction()
    {
        ...
        $this->getMassactionBlock()->addItem('enable', array(
            'label' => $h->__('Enable'),
            'url'   => $this->getUrl('adminhtml/mibizum_sync_attributebadge/massEnable'),
        ));
        $this->getMassactionBlock()->addItem('disable', array(
            'label' => $h->__('Disable'),
            'url'   => $this->getUrl('adminhtml/mibizum_sync_attributebadge/massDisable'),
        ));
        $this->getMassactionBlock()->addItem('delete', array(
            'label'   => $h->__('Remove'),
            'url'     => $this->getUrl('adminhtml/mibizum_sync_attributebadge/massDelete'),
    ...
    {
        return $this->getUrl('adminhtml/mibizum_sync_attributebadge/edit', array('id' => $row->getId()));
    }
    ...
    public function renderIconPreview($value, $row)
    {
        ...
        if ($svg !== '') {
            return '<span style="display:inline-block;width:32px;height:32px;color:' . $color . ';overflow:hidden;line-height:0;">'
                . preg_replace_callback('/<svg\b([^>]*)>/i', function ($m) {
                    $attrs = preg_replace('/\s(?:width|height|style)\s*=\s*"[^"]*"/i', '', $m[1]);
